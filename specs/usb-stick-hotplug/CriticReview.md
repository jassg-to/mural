...
| F11 | **Step 29's udev guards were two-thirds non-functional.** `ENV{ID_FS_TYPE}!="swap"` is a no-op (the existing `ID_FS_USAGE=="filesystem"` match already excludes swap), and "skip devices already in `/proc/self/mounts`" is not expressible in a udev rule without a helper script. Only `ATTRS{removable}=="1"` works — and it excludes USB SSDs, which is exactly the USB-boot case the guard was for. | tasks.md Step 29 (use the guard that works; state the exclusion rather than let it be discovered) |
